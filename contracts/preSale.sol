...
contract preSale is OwnerWithdrawable {
    ...
    struct BuyerTokenDetails {
        ...
        bool teamAllocation;
        bool teamClaimStart;
        bool launcVestingClaimed;
        uint256 monthlyVestingClaimed;
    }

    uint256 public launchVestingPercent;
    uint256 public monthlyVestingPercent;
    uint256 public teamVestingPercent;
    ...
    function setVestingPercents(
        uint256 _launchVestingPercent,
        uint256 _monthlyVestingPercent,
        uint256 _teamVestingPercent) external onlyOwner{
        launchVestingPercent = _launchVestingPercent;
        monthlyVestingPercent = _monthlyVestingPercent;
        // vestingBeginTime = block.timestamp;
        teamVestingPercent = _teamVestingPercent;
        }
    ...
    function stopSale() external onlyOwner saleDuration{
        // require(block.timestamp > preSaleStartTime && block.timestamp < preSaleEndTime, "PreSale: Sale hasn't started yet!");
    ...
    function buyToken(address _token, uint256 _amount) external payable saleDuration{
        ...
        buyersAmount[msg.sender].totalAmount = buyersAmount[msg.sender].totalAmount.add(saleTokenAmt);
        buyersAmount[msg.sender].startingAmount = buyersAmount[msg.sender].startingAmount.add(saleTokenAmt);
        // buyersAmount[msg.sender].tokenBuyTime = block.timestamp;
      
    }

    function withdrawToken()external{
        uint256 tokensforWithdraw = getAllocation(msg.sender);
        console.log(tokensforWithdraw);
        require(tokensforWithdraw > 0, "PreSale: No Tokens to withdraw!");
        buyersAmount[msg.sender].totalAmount -= tokensforWithdraw;
        IERC20Metadata(saleToken).safeTransfer(msg.sender, tokensforWithdraw);
    }

    //function to check allocation available for withdrawal
    function getAllocation(address user) public returns(uint256) {
        ...
        if(!buyersAmount[user].teamAllocation){

            if(!buyersAmount[user].launcVestingClaimed){
                buyersAmount[user].launcVestingClaimed = true;  
                return buyersAmount[user].startingAmount.mul(launchVestingPercent).div(100); 
            }
            else{
                // timeElapsed = block.timestamp.sub(buyersAmount[user].tokenBuyTime);
                // require(timeElapsed > (30*24*60*60), "PreSale: Monthly Vesting Period has not started yet!");
                boost = timeElapsed.div(30*24*60*60) - buyersAmount[user].monthlyVestingClaimed;
                console.log(boost);
                availableAllocation = buyersAmount[user].startingAmount.mul(boost).mul(monthlyVestingPercent).div(100);
                console.log(availableAllocation);
                buyersAmount[user].monthlyVestingClaimed = timeElapsed.div(30*24*60*60);
                console.log(buyersAmount[user].monthlyVestingClaimed);
                if(availableAllocation > buyersAmount[user].totalAmount){
                    return buyersAmount[user].totalAmount;
                }
                else{
            ...
            bool yearUnlocked = (timeElapsed >= 365*24*3600) ? true : false;
            require(yearUnlocked, "Team Allocation: You cannot claim your allocation before 1 year");
            
            if(!buyersAmount[user].teamClaimStart){
                // buyersAmount[user].monthlyVestingClaimed = timeElapsed.div(30*24*60*60);
                buyersAmount[user].teamClaimStart = yearUnlocked;
                return buyersAmount[user].startingAmount.mul(teamVestingPercent).div(100);
            }
            else{
                boost = timeElapsed.div(30*24*60*60) - buyersAmount[user].monthlyVestingClaimed;
                buyersAmount[user].monthlyVestingClaimed = timeElapsed.div(30*24*60*60);
                availableAllocation = buyersAmount[user].startingAmount.mul(teamVestingPercent).div(100);
                if(availableAllocation > buyersAmount[user].totalAmount){
                    return buyersAmount[user].totalAmount;
                }
                else{
    ...
    function setExternalAllocation(address[] memory user, uint256[] memory amount, bool[] memory teamAlloc)public onlyOwner{
        uint256 totalTokens;
        require(user.length == amount.length, "Presale: user & amount arrays length mismatch");
        require(user.length == teamAlloc.length, "Presale: user & teamAlloc arrays length mismatch");
        for(uint256 i = 0; i < user.length; i+=1){
            buyersAmount[user[i]].totalAmount = amount[i];
            buyersAmount[user[i]].startingAmount = amount[i];
            if(teamAlloc[i]){
                buyersAmount[user[i]].teamAllocation = teamAlloc[i];
                buyersAmount[user[i]].monthlyVestingClaimed = 12;
            }
            totalTokens += amount[i];
        }
        IERC20Metadata(saleToken).safeTransferFrom(msg.sender, address(this), totalTokens);
    }

    function getVestedTokens()external view returns(uint256){
        return (buyersAmount[msg.sender].startingAmount).sub(buyersAmount[msg.sender].totalAmount);
    }

    function getUnvestedTokens()external view returns(uint256){
        return buyersAmount[msg.sender].totalAmount;
    }
