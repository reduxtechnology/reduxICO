...
// 
    ...
    it("Should set correct Params for the PreSale contract", async function(){
        ...
        await presale.setVestingPercents(5, 7, 5);
    ...
    it("Should allow users to buy Sale Token.", async function(){
        ...
        // await expect(presale.connect(accountB).
        // buyToken(btoken.address, "30000000000000000000000"))
        ...
        expect(buyerAmt2.startingAmount).to.equal("3000000000000000000000");
        // expect(await buyerAmt11.startingAmount).to.equal("4007500000000000000000");
        expect(buyerAmt3.startingAmount).to.equal(String(10**18));
        expect(await atoken.balanceOf(presale.address)).to.equal("0");
    ...
    it("Should allow users to buy Sale Token with new rates.", async function(){
        ...
        expect(buyerAmt1.startingAmount).to.equal("2500000000000000000000");
        expect(buyerAmt2.startingAmount).to.equal("3666666666666666666666");
        expect(buyerAmt3.startingAmount).to.equal("1100000000000000000");
        ...
        expect(await atoken.balanceOf(accountA.address)).to.equal("90000000000000000000000");
    ...
    it("Should allow users to withdraw Sale Token.", async function(){
        ...
        launchPerc = await presale.launchVestingPercent();
        monthlyPerc = await presale.monthlyVestingPercent();

        totalBalA = await presale.buyersAmount(accountA.address);
        totalBalB = await presale.buyersAmount(accountB.address);
        totalBalC = await presale.buyersAmount(accountC.address);

        expBalA = totalBalA.startingAmount*(launchPerc)/100;
        expBalB = totalBalB.startingAmount*(launchPerc)/100;
        expBalC = totalBalC.startingAmount*(launchPerc)/100;
        ...
        // expect(await mytoken.balanceOf(accountA.address)).to.equal(String(expBalA));
        // expect(await mytoken.balanceOf(accountB.address)).to.equal(String(expBalB));
        // expect(await mytoken.balanceOf(accountC.address)).to.equal(String(expBalC));
        for(i = 0; i<20; i++){
            await advancetime(30 * 24 * 60 * 60);
            await advanceBlock();
            bal1 = await mytoken.balanceOf(accountA.address);
            await presale.connect(accountA).withdrawToken();
            bal2 = await mytoken.balanceOf(accountA.address);
            console.log(i)
            console.log(bal2-bal1)
            console.log(totalBalA.startingAmount*(monthlyPerc)/100);
            console.log(totalBalA.totalAmount/10^18);
            // expect(String(totalBalA.startingAmount-totalBalA.totalAmount)).to.equal((expBalA+totalBalA.startingAmount*(monthlyPerc)/100).toLocaleString('fullwide', { useGrouping: false }));
            // expect(await mytoken.balanceOf(accountA.address)).to.equal(String(expBalA+(totalBalA.startingAmount*(monthlyPerc)/100)));
        }
        return
        await advancetime(30 * 24 * 60 * 60);
        await advanceBlock();
        ...
        await expect(presale
            .connect(accountA)
            .withdrawToken()
            ).to.be.revertedWith("PreSale: No Tokens to withdraw!");
